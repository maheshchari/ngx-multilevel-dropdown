...
export class TreeviewItemComponent {
  ...
  getChildrenFilter = item => {
    if (item.filterText) {
      return item.children.filter((child) => child.text.indexOf(item.filterText) > -1);
    } else {
      return item.children;
    }
  }

  onSelectAll(item: TreeviewItem, checkbox) {
    item.children.forEach((child) => {
      child.checked = checkbox.checked;
      this.onChildCheckedChange(child, checkbox.checked);
    });
  }
